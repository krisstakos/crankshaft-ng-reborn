...
void fm_stream_stop()
{
    ...
    args << "-f";
    args << "aplay";
    process->start("pkill", args);
}
void musicVolume_clicked(bool status){
    ...
    process->start("vol", args);
}

//rtl_fm -f 96.3e6 -M wbfm -s 200000 -r 48000 - | aplay -r 48k -f S16_LE
void fm_stream_play(QString frequency)
{
    fm_stream_stop();
    QProcess *process = new QProcess();
    QStringList args;
    args << "-f" << frequency << "-M" << "wbfm" << "-s";
    args << "-s" << "200000" << "-r" << "48000" << "-";
    args << "|";
    args << "aplay" << "-r" << "48k" << "-f" << "S16-LE";
    process->start("rtl-fm", args);
}

// void fm_stream_play(QString url)
// {
//     QProcess *process = new QProcess();
//     QStringList args;
...
//     args << "!";
//     args << "rawaudioparse";
//     args << "use-sink-caps=false";
//     args << "format=pcm";
//     args << "pcm-format=s16le";
...
void f1x::openauto::autoapp::ui::MainWindow::on_pushButtonPlayerPlayList_clicked()
{
    ...
    QString data = playlist->currentMedia().canonicalUrl().toString();
    //we are expecting frequency:96.3e6 format
    if(data.contains("frequency")){ 
        fm_stream_play(data.split(':')[1]);
    }
    else{
        fm_stream_stop();
        player->play();
...
void f1x::openauto::autoapp::ui::MainWindow::on_pushButtonPlayerPrevBig_clicked()
{
    playlist->previous();
    if (playlist->currentIndex() != -1) {
        QString url = playlist->currentMedia().canonicalUrl().toString();
        if(data.contains("frequency")){ 
            fm_stream_play(data.split(':')[1]);
        }
        else{
...
void f1x::openauto::autoapp::ui::MainWindow::KeyPress(QString key)
{
    if (configuration_->playerButtonControl()) {
        ...
        //Volume keys
        if (key == "Q") { //Plus
            musicVolume_clicked(true);
        }
        if (key == "Z") { //Minus
            musicVolume_clicked(false);
        }
    }
}
    ...
    if (std::ifstream("/tmp/config_in_progress") || std::ifstream("/tmp/debug_in_progress") || std::ifstream("/tmp/enable_pairing")) {
        if (ui_->SysinfoTopLeft2->isVisible() == false) {
            if (std::ifstream("/tmp/config_in_progress")) {
                ...
                ui_->SysinfoTopLeft2->setText("Saving...");
